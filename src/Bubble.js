// Thanks https://github.com/maginth from Flacon
//
//	                                               (\_/)
//	  /\_/\                                        (^�^)
//	w( �u� )                                        ) (
...
/*
	type: 0->fleche droite 1->fleche courbe � base perpendiculaire 2->fleche courbe � base arrondie
	base: largeur de la base de la fleche
	marge: proportion de la longueur du bord portant la fleche laiss� vide entre la base de la fleche et le coin
	contour: �cart en pixel entre la pointe de la fleche et le cadre de l'�l�ment de destination;
*/
export function Bubble(type, base, marge, contour) {
    this.type = type;
    this.base = base;
    this.marge = marge;
    ...
    var fleche = [
        ...
        function (g, x1, y1, x2, y2, x3, y3) {
            ...
            vy = my + vy * scal;
            g.moveTo(x1, y1);
            g.quadraticCurveTo(vx, vy, x2, y2);
            g.quadraticCurveTo(vx, vy, x3, y3);
        },
        function (g, x1, y1, x2, y2, x3, y3) {
            var vx = y1 - y3,
                vy = x3 - x1,
            ...
            vy = my + vy * scal;
            g.moveTo(x1, y1);
            g.bezierCurveTo(mx, my, vx, vy, x2, y2);
            g.bezierCurveTo(vx, vy, mx, my, x3, y3);
        }
    ];

    //sc: �l�ment source; ds: �l�ment destination; canvas (facultatif): canvas � utiliser pour dessiner la fleche ou la redessiner
    this.bulleConnect = function (sc, ds, canvas) {
        ...
        var x0 = 0,
            ...
            x3,
            y3,
            sxx = sc.offsetLeft - blw,
            syy = sc.offsetTop - btw,
            sw = sc.offsetWidth + blw + brw,
            sh = sc.offsetHeight + btw + bbw,
            dw = ds.offsetWidth,
            dh = ds.offsetHeight,
            dxx = ds.offsetLeft,
            dyy = ds.offsetTop;
        ...
        function coin(b, bb) {
            var rd =
                scs["border" + (b ? "Top" : "Bottom") + (bb ? "Left" : "Right") + "Radius"].match(
                    /[\d\.]+(px|%)/g
                );
            rndx = parseFloat(rd[0]) || 0;
            if (rd[0].substr(-1) == "%") rndx *= sw / 100;
            rndy = parseFloat(rd[1] || rd[0]) || 0;
            if ((rd[1] || rd[0]).substr(-1) == "%") rndx *= sh / 100;
            rndx -= bb ? blw : brw;
            rndy -= b ? btw : bbw;
        }
        coin(top, left);
        ...
        if (ssy > basey + base && sy < -basey - base) {
            ...
            if (ssx > basex + base && sx < -basex - base) {
                ...
                if ((top ? -y2 : y2) < ax2)
                    y2 = top ? Math.max(-ax2, -sy - sh) : Math.min(ax2, -ssy + sh);
            }
            coin(top, !left);
            if (x0 > sw - rndx - base) {
        ...
        var cc = scs.backgroundColor,
            l = parseFloat(scs[borderPos + "Width"]) || 0,
            ll = l / 2;
        ...
        var xc = left ? w : 0,
            yc = top ? h : 0,
            xo = left ? -x0 + rdx : x0 + w - rdx,
            yo = top ? -y0 + rdy : y0 + h - rdy;
        rdx += 2 * (left ? blw : brw);
        rdy += 2 * (top ? btw : bbw);
        ...
        if (l) {
            g.strokeStyle = scs[borderPos + "Color"];
            g.stroke();
        }
